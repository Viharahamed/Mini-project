...
def create_sentiment_analyzer():
    ...
    try:
        # Try using a simpler model that doesn't require TensorFlow
        return pipeline(
            "sentiment-analysis",
            model="distilbert-base-uncased-finetuned-sst-2-english",
            framework="pt"  # Explicitly use PyTorch instead of TensorFlow
        )
...
def analyze_text_sentiment(text, analyzer):
    ...
    try:
        ...
        # Fallback to keyword analysis if no analyzer
...
def analyze_instagram_profile(username, analyzer, insta_context, retry_count=3, retry_delay=5):
    ...
    for attempt in range(retry_count):
        try:
            ...
            # Calculate overall risk score using the new risk_score metric
            risk_scores = {
            ...
            # Weight the components (bio carries more weight)
            overall_risk = (
            ...
            # Additional risk factors from keywords
            additional_risk_factors = detect_risk_keywords(profile.biography)
            keyword_risk = len(additional_risk_factors) * 0.1  # Each keyword adds 10% to risk

            # Combine ML-based risk with keyword-based risk
